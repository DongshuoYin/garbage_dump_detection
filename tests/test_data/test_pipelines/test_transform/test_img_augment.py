...
def test_imequalize(nb_rand_test=100):
    ...
    def _imequalize(img):
        # equalize the JPEGImages using PIL.ImageOps.equalize
    ...
    # test equalize with randomly sampled JPEGImages.
...
def test_adjust_brightness(nb_rand_test=100):
    ...
    def _adjust_brightness(img, factor):
        # adjust the brightness of JPEGImages using
...
def test_adjust_contrast(nb_rand_test=100):
    ...
    for _ in range(nb_rand_test):
        ...
        # a color JPEGImages to gray JPEGImages using mmcv or PIL.
        np.testing.assert_allclose(
            transform_module(copy.deepcopy(results))['img'].astype(np.int32),
