...
def mask_target(pos_proposals_list, pos_assigned_gt_inds_list, gt_masks_list,
    ...
    Returns:
        list[Tensor]: Mask target of each JPEGImages.
    ...
    Example:
        ...
        >>> # Positive proposals (tl_x, tl_y, br_x, br_y) for each JPEGImages
        >>> pos_proposals_list = [
        >>>     torch.Tensor([
        ...
        >>>     ]),
        >>> ]
        >>> # Corresponding class index for each proposal for each JPEGImages
        >>> pos_assigned_gt_inds_list = [
        >>>     torch.LongTensor([7, 0]),
        >>>     torch.LongTensor([5, 4, 1]),
        >>> ]
        >>> # Ground truth mask for each true object for each JPEGImages
        >>> gt_masks_list = [
...
def mask_target_single(pos_proposals, pos_assigned_gt_inds, gt_masks, cfg):
    """Compute mask target for each positive proposal in the JPEGImages.
    ...
    Returns:
        Tensor: Mask target of each positive proposals in the JPEGImages.
    ...
    Example:
        ...
        >>> rng = np.random.RandomState(0)
        >>> # Masks for each ground truth box (relative to the JPEGImages)
        >>> gt_masks_data = rng.rand(3, H, W)
        >>> gt_masks = BitmapMasks(gt_masks_data, height=H, width=W)
        >>> # Predicted positive boxes in one JPEGImages
