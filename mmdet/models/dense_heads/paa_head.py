...
def levels_to_images(mlvl_tensor):
    """Concat multi-level feature maps by JPEGImages.

    [feature_level0, feature_level1...] -> [feature_image0, feature_image1...]
    Convert the shape of each element in mlvl_tensor from (N, C, H, W) to
    (N, H*W , C), then split the element to N elements with shape (H*W, C), and
    concat elements in same JPEGImages of all level along first dimension.
...
class PAAHead(ATSSHead):
    ...
    def get_pos_loss(self, anchors, cls_score, bbox_pred, label, label_weight,
        ...
        Args:
            ...
            cls_score (Tensor): Box scores of single JPEGImages with shape
                (num_anchors, num_classes)
            bbox_pred (Tensor): Box energies / deltas of single JPEGImages
        ...
        Returns:
            Tensor: Losses of all positive samples in single JPEGImages.
        """
        if not len(pos_inds):
