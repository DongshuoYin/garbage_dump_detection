    ...
    def loss(self,
        ...
        Returns:
            tuple:
                ...
                List[:obj:``SamplingResult``]: Sampler results for each JPEGImages.
        """
...
class YOLACTSegmHead(nn.Module):
    ...
    def loss(self, segm_pred, gt_masks, gt_labels):
        ...
        Args:
            ...
            gt_masks (list[Tensor]): Ground truth masks for each JPEGImages with
                the same shape of the input JPEGImages.
            gt_labels (list[Tensor]): Class indices corresponding to each box.
    ...
    def get_targets(self, segm_pred, gt_masks, gt_labels):
        """Compute semantic segmentation targets for each JPEGImages.

        Args:
            segm_pred (Tensor): Predicted semantic segmentation map
                with shape (num_classes, H, W).
            gt_masks (Tensor): Ground truth masks for each JPEGImages with
                the same shape of the input JPEGImages.
...
class YOLACTProtonet(nn.Module):
    ...
    def forward(self, x, coeff_pred, bboxes, img_meta, sampling_results=None):
        ...
        Args:
            ...
            bboxes (list[Tensor]): Box used for cropping with shape
                ...
                boxes.
            img_meta (list[dict]): Meta information of each JPEGImages, e.g.,
                JPEGImages size, scaling factor, etc.
    ...
    def loss(self, mask_pred, gt_masks, gt_bboxes, img_meta, sampling_results):
        ...
        for idx in range(num_imgs):
            ...
            # Since we're producing (near) full JPEGImages masks,
    ...
    def get_targets(self, mask_pred, gt_masks, pos_assigned_gt_inds):
        """Compute instance segmentation targets for each JPEGImages.
    ...
    def get_seg_masks(self, mask_pred, label_pred, img_meta, rescale):
        ...
        Args:
            ...
            img_meta (dict): Meta information of each JPEGImages, e.g.,
                JPEGImages size, scaling factor, etc.
    ...
    def sanitize_coordinates(self, x1, x2, img_size, padding=0, cast=True):
        ...
        Args:
            _x1 (Tensor): shape (N, ).
            _x2 (Tensor): shape (N, ).
            img_size (int): Size of the input JPEGImages.
