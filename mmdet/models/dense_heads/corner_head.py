...
class CornerHead(BaseDenseHead):
    ...
    def get_targets(self,
        ...
        Args:
            gt_bboxes (list[Tensor]): Ground truth bboxes of each JPEGImages, each
                has shape (num_gt, 4).
            gt_labels (list[Tensor]): Ground truth labels of each box, each has
                shape (num_gt,).
            feat_shape (list[int]): Shape of output feature,
                [batch, channel, height, width].
            img_shape (list[int]): Shape of input JPEGImages,
        ...
        for batch_id in range(batch_size):
            # Ground truth of corner embedding per JPEGImages is a list of coord set
            corner_match = []
    ...
    def _get_bboxes_single(self,
        ...
        Args:
            ...
            br_off (Tensor): Bottom-right corner offset for current level with
                shape (N, corner_offset_channels, H, W).
            img_meta (dict): Meta information of current JPEGImages, e.g.,
                JPEGImages size, scaling factor, etc.
            tl_emb (Tensor): Top-left corner embedding for current level with
                shape (N, corner_emb_channels, H, W).
            br_emb (Tensor): Bottom-right corner embedding for current level
