...
class CornerNet(SingleStageDetector):
    ...
    def merge_aug_results(self, aug_results, img_metas):
        ...
        Args:
            aug_results (list[list[Tensor]]): Det_bboxes and det_labels of each
                JPEGImages.
            img_metas (list[list[dict]]): Meta information of each JPEGImages, e.g.,
    ...
    def aug_test(self, imgs, img_metas, rescale=False):
        ...
        Note:
            ``imgs`` must including flipped JPEGImages pairs.

        Returns:
            list[list[np.ndarray]]: BBox results of each JPEGImages and classes.
                The outer list corresponds to each JPEGImages. The inner list
                corresponds to each class.
        """
        img_inds = list(range(len(imgs)))

        assert img_metas[0][0]['flip'] + img_metas[1][0]['flip'], (
            'aug test must have flipped JPEGImages pair')
        aug_results = []
