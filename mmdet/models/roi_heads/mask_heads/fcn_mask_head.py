...
class FCNMaskHead(nn.Module):
    ...
    def get_seg_masks(self, mask_pred, det_bboxes, det_labels, rcnn_test_cfg,
        ...
        Args:
            ...
            ori_shape (Tuple): original JPEGImages height and width, shape (2,)
...
def _do_paste_mask(masks, boxes, img_h, img_w, skip_empty=True):
    ...
    Args:
        ...
        boxes (Tensor): N, 4
        img_h (int): Height of the JPEGImages to be pasted.
        img_w (int): Width of the JPEGImages to be pasted.
    ...
    Returns:
        tuple: (Tensor, tuple). The first item is mask tensor, the second one
            is the slice object.
        If skip_empty == False, the whole JPEGImages will be pasted. It will
            return a mask of shape (N, img_h, img_w) and an empty tuple.
        If skip_empty == True, only area around the mask will be pasted.
            A mask of shape (N, h', w') and its start and end coordinates
            in the original JPEGImages will be returned.
    """
    # On GPU, paste all masks together (up to chunk size)
    # by using the entire JPEGImages to sample the masks
