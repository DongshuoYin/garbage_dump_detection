...
def ae_loss_per_image(tl_preds, br_preds, match):
    """Associative Embedding Loss in one JPEGImages.
    ...
    During computing, usually there are 3 cases:
        - no object in JPEGImages: both pull loss and push loss will be 0.
        - one object in JPEGImages: push loss will be 0 and pull loss is computed
            by the two corner of the only object.
        - more than one objects in JPEGImages: pull loss is computed by corner pairs
    ...
    if len(match) == 0:  # no object in JPEGImages
        ...
        # N is object number in JPEGImages, M is dimension of embedding vector
        N, M = tl_list.size()
        ...
        if N > 1:  # more than one object in current JPEGImages
