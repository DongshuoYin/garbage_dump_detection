...
@PIPELINES.register_module()
class Shear(object):
    """Apply Shear Transformation to JPEGImages (and its corresponding bbox, mask,
    segmentation).

    Args:
        level (int | float): The level should be in range [0,_MAX_LEVEL].
        img_fill_val (int | float | tuple): The filled values for JPEGImages border.
            If float, the same fill value will be used for all the three
            channels of JPEGImages. If tuple, the should be 3 elements.
...
@PIPELINES.register_module()
class Rotate(object):
    """Apply Rotate Transformation to JPEGImages (and its corresponding bbox, mask,
    segmentation).
    ...
    Args:
        ...
        center (int | float | tuple[float]): Center point (w, h) of the
            rotation in the source JPEGImages. If None, the center of the
            JPEGImages will be used. Same in ``mmcv.imrotate``.
        img_fill_val (int | float | tuple): The fill value for JPEGImages border.
            If float, the same value will be used for all the three
            channels of JPEGImages. If tuple, the should be 3 elements (e.g.
            equals the number of channels for JPEGImages).
        seg_ignore_label (int): The fill value used for segmentation map.
            Note this value must equals ``ignore_label`` in ``semantic_head``
            of the corresponding config. Default 255.
...
class Translate(object):
    ...
    Args:
        ...
        img_fill_val (int | float | tuple): The filled value for JPEGImages
            border. If float, the same fill value will be used for all
            the three channels of JPEGImages. If tuple, the should be 3
            elements (e.g. equals the number of channels for JPEGImages).
        seg_ignore_label (int): The fill value used for segmentation map.
            Note this value must equals ``ignore_label`` in ``semantic_head``
            of the corresponding config. Default 255.
    ...
    def _translate_bboxes(self, results, offset):
        ...
        for key in results.get('bbox_fields', []):
            ...
            # the boxs translated outside of JPEGImages will be filtered along with
    ...
    def _filter_invalid(self, results, min_size=0):
        """Filter bboxes and masks too small or translated out of JPEGImages."""
        bbox2label, bbox2mask, _ = bbox2fields()
        for key in results.get('bbox_fields', []):
            bbox_w = results[key][:, 2] - results[key][:, 0]
            bbox_h = results[key][:, 3] - results[key][:, 1]
...
class ColorTransform(object):
    """Apply Color transformation to JPEGImages. The bboxes, masks, and
    ...
    def _adjust_color_img(self, results, factor=1.0):
        """Apply Color transformation to JPEGImages."""
        for key in results.get('img_fields', ['img']):
            # NOTE defaultly the JPEGImages should be BGR format
            img = results[key]
...
class EqualizeTransform(object):
    """Apply Equalize transformation to JPEGImages. The bboxes, masks and
    segmentations are not modified.
    ...
    def _imequalize(self, results):
        """Equalizes the histogram of one JPEGImages."""
        for key in results.get('img_fields', ['img']):
            img = results[key]
...
class BrightnessTransform(object):
    """Apply Brightness transformation to JPEGImages. The bboxes, masks and
    segmentations are not modified.
    ...
    def _adjust_brightness_img(self, results, factor=1.0):
        """Adjust the brightness of JPEGImages."""
        for key in results.get('img_fields', ['img']):
            img = results[key]
...
class ContrastTransform(object):
    """Apply Contrast transformation to JPEGImages. The bboxes, masks and
    segmentations are not modified.
    ...
    def _adjust_contrast_img(self, results, factor=1.0):
        """Adjust the JPEGImages contrast."""
        for key in results.get('img_fields', ['img']):
            img = results[key]
